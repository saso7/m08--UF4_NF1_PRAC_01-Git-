...
class OrdersController extends Controller
{
    ...
    public function edit(Request $request)
    {
        Validator::make($request->all(), [
            'newStatus' => ['required', 'string', 'in:pending,completed,delivered'],
        ])->validate();
    ...
    public function indexBasket(){
        ...
        $orders = Orders::all()->where('users_id',$user_id)->where('status','pending');
        // dd($orders);
        if($orders->isEmpty()){
            ...
            // dd($p);
            // dd($orders[$p]->getOriginal('id'));
            $ordersItems = DB::table('orders_items')->where('order_id',$orders[$p]->getOriginal('id'))->get();
            // dd($ordersItems);
            $productNames = [];
            $productPrices = [];
    ...
    public function buy($orderId){
        ...
        // $productAmount = DB::table('products')->select('amount')->where('name',$request->name)->get();
        DB::table('orders')->where('id',$orderId)->update([
            'status' => 'completed',
            'updated_at' => now(),
        ]);
        
        // dd($productAmount[0]->amount);
        // return redirect()->route('basket');
        // return redirect()->action([OrdersController::class, 'indexBasket']);
